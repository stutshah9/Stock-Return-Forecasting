    ...
    for candidate in (
        ...
        if candidate.is_file():
            ...
            for coverage in ("80", "90", "95"):
                ...
                if interval_col not in predictions.columns and lower_col in predictions.columns and upper_col in predictions.columns:
                    predictions[interval_col] = predictions.apply(
                        lambda row: f"[{float(row[lower_col]):.4f}, {float(row[upper_col]):.4f}]",
                        axis=1,
                    )
                if width_col not in predictions.columns and lower_col in predictions.columns and upper_col in predictions.columns:
                    predictions[width_col] = predictions[upper_col].astype(float) - predictions[lower_col].astype(float)
            return predictions
...
def _render_prediction(year: str, ticker: str, date: str) -> tuple[str, pd.DataFrame, pd.DataFrame]:
    ...
    compact_table["predicted_return"] = compact_table["predicted_return"].map(lambda value: f"{float(value):.4f}")
    compact_table["prediction_error"] = compact_table["prediction_error"].map(lambda value: f"{float(value):.4f}")
    detail_columns = [
        "method",
        "coverage_80_lower",
        "coverage_80_upper",
    ...
    detail_table = filtered[detail_columns].copy()
    for column_name in detail_table.columns:
        if column_name != "method":
            detail_table[column_name] = detail_table[column_name].map(lambda value: f"{float(value):.4f}")
...
def build_app() -> gr.Blocks:
    ...
    with gr.Blocks(title="Earnings Prediction Viewer") as app:
        ...
        with gr.Row():
            ...
            ticker = gr.Dropdown(choices=default_ticker_choices, value=default_ticker, label="Ticker")
        ...
        app.load(_render_prediction, inputs=[year, ticker, date], outputs=[summary, ranges, details])

...
def _prediction_rows(
    ...
    rows: list[dict[str, float | str | int]] = []
    for event, output, label, regime, ticker in zip(events, outputs, labels, regimes, tickers):
        event_metadata = _event_calibration_metadata(event)
...
def evaluate(config_path: str) -> None:
    ...
    cal_metadata = [_event_calibration_metadata(event) for event in cal_events]
    predictor.calibrate(
    ...
    test_metadata = [_event_calibration_metadata(event) for event in test_events]
